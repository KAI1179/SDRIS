...
def inference(test_loader, model, args):
    ...
    for img, param in tbar:
        ...
        for sent in param['sents']:
            ...
            iou = np.sum(inter) / (np.sum(union) + 1e-6)
            iou_list.append(iou)
            cum_I += np.sum(inter)      # Used for oIoU
            cum_U += np.sum(union)      # Used for oIoU
            # dump prediction
            if args.visualize:
    ...
    logger.info('oIoU={:.2f}'.format(oiou))
    for k, v in prec.items():
        logger.info('{}: {:.2f}.'.format(k, 100.*v))

    return miou.item(), oiou, prec
