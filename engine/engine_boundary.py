...
def train(train_loader, model, optimizer, scheduler, scaler, epoch, args):
    ...
    loss_meter = AverageMeter('Loss_BCE', ':2.4f')
    loss_meter_dist = AverageMeter('Loss_Dist', ':2.4f')
    iou_meter = AverageMeter('IoU', ':2.2f')
    pr_meter = AverageMeter('Prec@50', ':2.2f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, lr, loss_meter, loss_meter_dist, iou_meter, pr_meter],
    ...
    for i, (image, text, target) in enumerate(train_loader):
        ...
        # ------------------ Forward ZHY --------------------
        with amp.autocast():
            pred, target, loss_dict = model(image, text, target)
        ...
        dist.all_reduce(loss.detach())      # loss的全局平均
        # dist.all_reduce(loss_dict["loss_dist"].detach())      # loss的全局平均
        dist.all_reduce(iou)
        dist.all_reduce(pr5)
        
        # _zhy_
        
        # loss = loss / dist.get_world_size()                        # 返回分布式组中的进程数
        loss_bce = loss_dict["loss_bce"] / dist.get_world_size()     # 返回分布式组中的进程数
        loss_dist = loss_dict["loss_dist"] / dist.get_world_size()   # 返回分布式组中的进程数

        iou = iou / dist.get_world_size()
        pr5 = pr5 / dist.get_world_size()
        
        # _zhy_
        # loss_meter.update(loss.item(), image.size(0))             # image.size(0) = B
        loss_meter.update(loss_bce.item(), image.size(0))           # image.size(0) = B
        loss_meter_dist.update(loss_dist.item(), image.size(0))     # image.size(0) = B
                ...
                wandb.log(
                    {
                        ...
                        "training/loss_dist": loss_meter_dist.val,      # _zhy_
