...
class ModifiedResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.layer2(x)
        x3 = self.layer3(x2)
        x4 = self.layer4(x3)        # (b, 2048, 13, 13)
        x4 = self.attnpool(x4)      # (b, 1024, 13, 13)

        return (x2, x3, x4), self.gap(x4)
...
class CLIP(nn.Module):
    def __init__(
            ...
            vision_layers: Union[Tuple[int, int, int, int], int],       # Tuple or Int
            vision_width: int,
        ...
        if isinstance(vision_layers, (tuple, list)):    # Check if x is one of the types
            vision_heads = vision_width * 32 // 64
            self.visual = ModifiedResNet(layers=vision_layers,      # List
